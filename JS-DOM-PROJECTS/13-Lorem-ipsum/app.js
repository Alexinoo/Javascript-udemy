...
// select elements
const formElmnt = document.querySelector('.lorem-form')
const articleElmnt = document.querySelector('.lorem-text')
const amountInputElmnt = document.getElementById('amount')

// Event Listeners
formElmnt.addEventListener('submit',generateLoremText)
...
function generateLoremText(e){
  ...
  let value = parseInt(amountInputElmnt.value)

  let randomNumber = Math.floor(Math.random() * textArray.length)
  
  // check for value input (value > 9 ) || value <= 0 || value === isNaN
  if( isNaN(value) || value > textArray.length || value <= 0 ){
    
      articleElmnt.innerHTML = `<p class="result">${textArray[randomNumber]}</p>`
  }else{
    ...
    */    
      ...
      loremIpsumText = loremIpsumText.map(function(paragraph){
        return `<p class="result">${paragraph}</p>`
      }).join('')

      articleElmnt.innerHTML = loremIpsumText
  }
